    ...
    for _, data in pairs(tbl) do
      ...
      if ids and #ids > 0 then
        if data.nameMode then
          ...
          if n then ns._raidBuffWatch.name[n] = true end
        else
          for _, id in ipairs(ids) do ns._raidBuffWatch.spellId[id] = true end
        end
      end
    end
  end
...
function ns.HandleRaidBuff_UNIT_AURA(unit, updateInfo)
  ...
  if not ns._raidBuffWatch then ns.RebuildRaidBuffWatch() end
...
local function addEntry(rowKey, data, catName)
  ...
  if catName == "RAID_BUFFS" and (data.count == nil) and (data.check ~= "player") and (not entry.centerText or entry.centerText == "") then
    local units = buildUnits()
